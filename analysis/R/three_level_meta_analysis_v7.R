# ============================================================================
# Three-Level Meta-Analysis: GenAI Effectiveness in Higher Education (V7)
# ============================================================================
# Author: Hosung You
...
#   - Added ES_Type classification column
#   - Added SE_g_verified and Variance_g columns
#   - V7: 365 effect sizes, 66 studies
# ============================================================================
...
# Load the V7 dataset
data <- read.csv("/Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/data/03_final/GenAI_MetaAnalysis_v7.csv")
...
cat(rep("=", 70), "\n", sep="")
cat("V7 META-ANALYSIS: DATA SUMMARY\n")
cat(rep("=", 70), "\n", sep="")
cat("Dataset version: V7 (2026-01-26)\n")
...
cat("Pre-test effect sizes removed: 10 (from V6)\n")
...
# ============================================================================
# 5. ES_TYPE MODERATOR ANALYSIS (NEW IN V7)
# ============================================================================

cat("\n", rep("=", 70), "\n", sep="")
cat("MODERATOR ANALYSIS: ES_TYPE (NEW IN V7)\n")
cat(rep("=", 70), "\n", sep="")

# Check ES_Type distribution
cat("\nES_Type distribution:\n")
print(table(data_valid$es_type, useNA = "ifany"))

# Filter to most common ES types
data_es_type <- data_valid %>%
  filter(!is.na(es_type)) %>%
  group_by(es_type) %>%
  filter(n() >= 5) %>%  # At least 5 effect sizes per type
  ungroup()

if(nrow(data_es_type) > 10 && length(unique(data_es_type$es_type)) > 1) {
  model_es_type <- rma.mv(
    yi = hedges_g,
    V = se_g^2,
    mods = ~ es_type - 1,
    random = ~ 1 | study_id/outcome_id,
...
# ============================================================================
# 7. ADDITIONAL MODERATOR ANALYSES
# ============================================================================
...
dir.create("/Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/analysis/output/v7_results",
           recursive = TRUE, showWarnings = FALSE)

# Create forest plot
png("/Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/analysis/output/v7_results/forest_plot_v7.png",
    width = 12, height = max(8, nrow(study_summary) * 0.3),
    units = "in", res = 150)
...
forest(
  ...
  xlim = c(-4, 5),
  xlab = "Hedges' g",
  main = "Forest Plot: GenAI Effects on Learning Outcomes in Higher Education (V7)"
)
...
cat("Forest plot saved to: /Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/analysis/output/v7_results/forest_plot_v7.png\n")

# ============================================================================
# 11. FUNNEL PLOT
# ============================================================================

png("/Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/analysis/output/v7_results/funnel_plot_v7.png",
...
plot(data_valid$hedges_g, data_valid$se_g,
     ...
     xlab = "Hedges' g",
     ylab = "Standard Error",
     main = "Funnel Plot: Publication Bias Assessment (V7)",
...
cat("Funnel plot saved to: /Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/analysis/output/v7_results/funnel_plot_v7.png\n")
...
saveRDS(results_summary, "/Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/analysis/output/v7_results/meta_analysis_v7_results.rds")

# ============================================================================
# 13. GENERATE MARKDOWN SUMMARY
# ============================================================================

sink("/Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/analysis/output/v7_results/meta_analysis_v7_results.md")

cat("# Three-Level Meta-Analysis Results: GenAI in Higher Education (V7)\n\n")
cat("**Analysis Date:**", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("**Dataset:** GenAI_MetaAnalysis_v7.csv\n")
cat("**Analysis Script:** three_level_meta_analysis_v7.R\n\n")

cat("---\n\n")

cat("## Executive Summary\n\n")
cat("This V7 analysis includes methodological improvements from V6:\n")
cat("- Removed 10 pre-test effect sizes (methodological correction)\n")
cat("- Added ES_Type classification for effect size calculation methods\n")
cat("- Added verified SE and variance columns\n")
cat("- V7 dataset: 365 effect sizes from 66 studies\n\n")

cat("The analysis uses a three-level multilevel modeling approach to account for multiple effect sizes nested within studies.\n\n")
...
cat("|------|-------------|\n")
cat("| `forest_plot_v7.png` | Forest plot of study-level effect sizes |\n")
cat("| `funnel_plot_v7.png` | Funnel plot for publication bias visualization |\n")
cat("| `meta_analysis_v7_results.rds` | R data object with full results |\n")
cat("| `meta_analysis_v7_results.md` | This summary file |\n\n")

sink()

cat("\nMarkdown summary saved to: /Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/analysis/output/v7_results/meta_analysis_v7_results.md\n")

cat("\n", rep("=", 70), "\n", sep="")
cat("V7 ANALYSIS COMPLETE\n")
cat(rep("=", 70), "\n", sep="")
cat("Results saved to: /Volumes/External SSD/Projects/GenAI-HE-Review-AIMC/analysis/output/v7_results/\n")
